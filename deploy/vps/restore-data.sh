...
docker exec -i content-funnel-postgres psql -v ON_ERROR_STOP=0 -U "$POSTGRES_USER" -d "$POSTGRES_DB" < "$BUNDLE/db-dumps/content_funnel.sql"
source "$REPO/apps/dgtl-os/local/deploy/docker/.env"
echo "→ dgtlkb into dgtl-pgvector ($POSTGRES_DB as $POSTGRES_USER)"
docker exec -i dgtl-pgvector psql -v ON_ERROR_STOP=0 -U "$POSTGRES_USER" -d "$POSTGRES_DB" < "$BUNDLE/db-dumps/dgtlkb.sql"
