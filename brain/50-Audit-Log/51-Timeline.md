...
- **2026-08-14 — DGTL Core Stage 4 implemented on `claude/dgtl-core-stage-4-95myaw`** (from merged Stage 3 checkpoint `main@33c0bde`, PR #27): the Worklog delivery operations bridge. Extracted the single shared Worklog HTTP client to `apps/worklog/client/worklog-client.mjs` (MCP now re-wraps it); added migration `013_worklog_operations_phase_4.sql` (`integration_operations` + `external_links` lifecycle columns, no Worklog data copies); built `platform/lib/stage4/` (connector registry, server-configured team-bound connector, matching, link retire/revive/repair with history, approved/checksummed/idempotent operations with `outcome_unknown` quarantine and deterministic marker reconciliation, freshness-stamped read-through snapshots, provenance-preserving digest pass-through); added `/operations/worklog` plus Company/Opportunity Worklog panels; MCP gained `worklog_list_clients` and `worklog_client_digest`. Verified: 22/22 stage-4 tests, 445/445 platform suite, 58-page production build, 375/375 Worklog suite, migrations 001–013 rehearsed with stable repeat, and `npm run rehearse:stage4` green against a **real local Worklog server** — project + implicit client + 3 marked tasks created over the real HTTP API, duplicate prevention proven via Worklog's own export, member-logged time read through (180 min, 15% budget), 13-row digest provenance, and the full failure catalogue (ambiguous, lost-response reconcile, stale/repair, auth, unreachable, Worklog-permission denial, cross-team, tamper). CI gate gains explicit Stage 4 tests and the real-Worklog rehearsal. Not merged; not deployed; production Worklog untouched and unconfigured.

- **2026-08-14 — DGTL Core Stage 5 implemented on `codex/home-command-center-phase-5`** (from squash-merged Stage 4 checkpoint `main@2fa56f9`, PR #28, remote `Required DGTL Core checkpoint` green): the HOME command center. `/home` renders one server-aggregated snapshot (attention / today / approvals / pipeline / delivery / generation / system health / recent activity / quick actions) as a pure projection over canonical Stage 1–4 state — no migration, no notifications table, no persisted dashboard state. New read-only aggregation queries only (team activities, opportunity stage aggregates with known-vs-unknown value, message state aggregates, bounded search). Deterministic attention priority classes (critical → action_required → due_today → upcoming), per-section degradation (a hung Worklog cannot blank the dashboard; probes coalesced and 3 s-bounded), grouped sidebar (HOME/GROW/CREATE/OPERATE), ⌘K command palette over session-scoped `/api/core/search`, and the app-host root redirect moved `/admin` → `/home` with tenant-host behavior regression-pinned. Verified: 17/17 stage-5 tests, 462/462 platform suite, production build, and `npm run rehearse:stage5` proving projection semantics on disposable PG + a real local Worklog (6 attention items → real resolutions in authoritative systems → 2; delivery summary appeared with freshness; no dashboard table exists). CI gate gains the stage-5 tests and rehearsal. Not merged; not deployed; commercial email disabled; production Worklog untouched.

- **2026-08-14 — DGTL Core Stage 6 implemented on `codex/dgtl-chat-command-layer-phase-6`** (from squash-merged Stage 5 checkpoint `main@879cc3e`, PR #29, remote `Required DGTL Core checkpoint` green): DGTL.chat, the first conversational interface. Architecture: the model interprets and proposes; **Core validates, authorizes and executes**. `ChatModelAdapter` seam (deterministic rule-based adapter as the CI acceptance authority; optional Anthropic adapter through the shared transport's new API-key-only `runChatToolTurn` — the subscription/Agent-SDK path is deliberately excluded because it bypasses permissions); static server-owned 16-tool registry (12 READ over Stage 1–5 services, 4 PREPARE producing ActionProposals; approvals surfaced never performed; email-send/deploy/delete/SQL/shell/HTTP are not tools and unknown ids fail closed with audit); strict arg validation (unknown keys rejected — a model-supplied `teamId` is an error); versioned trusted system policy with an explicit untrusted-data hierarchy; migration `014_dgtl_chat_command_layer.sql` (threads/messages/tool-runs/proposals, CAS states, payload hash + precondition snapshot + 24 h expiry, one live proposal per exact payload); explicit human confirmation only (thread owner, `requireCoreWrite`, roles re-checked) with execution-time revalidation → `stale` with no mutation; bounded turns (4k input, 8 iterations, 6 tool calls, 60 s, 10 turns/min); no chain-of-thought stored; SourceRefs only from tool results; `/chat` UI + ⌘K "Ask DGTL" + contextual entries on Company/Opportunity. Verified: 25/25 stage-6 tests, **487/487 platform suite**, production build, 0 production advisories, migrations 001–014 rehearsed, 375/375 Worklog suite, regression rehearsals 2–5 green, and `npm run rehearse:stage6` (scenarios A–J incl. prompt injection inert, hostile scripted model rejected at every layer, stale-proposal no-mutation, provider-failure isolation) green on disposable PG + a real local Worklog with **no AI provider**. CI gate gains the stage-6 tests and rehearsal. Not merged; not deployed; no production email; production Worklog untouched.

- **2026-08-15 — DGTL Core Stage 6.5 deployment readiness prepared on `ops/dgtl-chat-internal-alpha`** (from merged Stage 6 `main@239587e`, PR #30, remote checkpoint green on the merged SHA). Audited the real topology: production is the VPS Docker/Coolify stack at `/opt/content-checkout-funnel` (Next standalone container + postgres:17 container behind Traefik), still running pre-Core `main@32c9f73`, so promotion applies migrations 009–014 to the production DB. Repo fixes: docker-compose gains the Stage 4/6 env passthrough (`CORE_WORKLOG_*`, `CORE_CHAT_*`), a `dgtl.chat` Traefik router on the same container (inert until DNS cutover), a container healthcheck, and `CORE_RELEASE_SHA`; authenticated `/api/core/health` now reports the release identity; baseline security headers (nosniff, referrer-policy, XFO SAMEORIGIN — CSP deliberately deferred as route-aware follow-up) added in `next.config.mjs`; both legacy `apps/dgtl-os` AI proxies flipped from wildcard CORS to deny-by-default `ALLOWED_ORIGINS`; new `tests/stage6-5-internal-alpha.test.js` (5 tests incl. dgtl.chat-is-unclaimed pin) and CI steps. Verified: 492/492 platform suite, production build, migrations 001–014 rehearsal green with stable repeat, and a production-shaped smoke on the built server (anonymous /home //chat/entity/API all redirect to login; `Secure; HttpOnly; SameSite=lax` cookie; logout invalidates; headers present; `Host: dgtl.chat` lands on /home; health shows release + transport `test` + all four send gates closed). DNS audit: dgtl.chat apex+www at parking IP `2.57.91.91`; cutover plan + rollback documented. Full gate-ordered runbook: `docs/operations/dgtl-chat-internal-alpha-runbook.md`. Deployment itself is blocked on external steps (VPS SSH for fresh backup/restore rehearsal + deploy, registrar for the one A record, Worklog admin for the integration account, provider key decision) — documented as exact manual instructions. No production system was touched.

- **2026-08-17 — DGTL brand-kit alignment for the authenticated OS UI on `ops/dgtl-brand-alignment`** (from `main@cfa382b`). The OS worked but had drifted from the brand kit. Introduced `platform/app/dgtl-tokens.css` as THE canonical token layer for every authenticated surface (Core, admin shell, login), mirroring the kit so future syncs are a readable diff; `dgtl-admin.css` and `core.css` now reference it and declare no brand literals, enforced by a new `tests/brand-tokens.test.js`. Tracing the cascade found two real leaks: `--brand` is `:root`-baked and was rendering literal legacy blue `#0071e3` in the tenant-builder selection states inside the gold app, and `--font-display` baked to Geist so the admin's KPI numbers were not Manrope — both re-pointed. (The `color-mix()` derived tokens were NOT broken: custom properties substitute at computed-value time on the element, and both palette blocks match the same element, so they already resolved against the winning black/gold ladder — they were deliberately left alone. `--glow`/`--shadow-brand` are switched off with a transparent shadow rather than `none`, which would invalidate the box-shadow lists they feed.) Core surface brought to the kit's app recipes: gold rationed to the primary action / active nav / counts / proposal card (it previously turned gold on *every* button, record-link, table-link and chat-ref hover); `Status` moved off gold onto the functional palette via a new `lib/core/statusTone.js` mapping the real vocabularies incl. HOME severities, with `neutral` fallback for stages/kinds/unknown DB strings; the missing third text level (#D0D0D0) applied so reading copy is no longer dim #8a8a8a (~3.5:1 on black); type raised from 31-of-60 sizes under 12px to the kit's app scale; the universal gold focus ring added (Core had **no** `:focus-visible` styling at all — an a11y gap); cards/tables/KPIs/empty states to recipe; the two dangling selectors filled (`.core-cmdk__ask` had no rule despite being the palette's primary affordance, and `chat-proposal.is-*` had rules only for `is-executed`). Manrope made deliberate via one shared preloaded `lib/fonts.js` instance — Core previously resolved it only because the root layout happens to put the variable on `<html>` with `preload:false`. `styles.css` `:root` untouched, so the public funnels have zero blast radius. Verified: 499/499 tests, production build, and Playwright captures on disposable PG with seeded fixtures asserting black ground, Manrope, 16px cards / 7px controls, `#2a2a2a` borders, the gold focus ring, three distinct status colours, and **zero** legacy-blue elements.

- **2026-08-17 — ⌘K made global, and the app's canonical-host move to `os.dgtl.ltd` prepared** (`ops/global-command-palette`, `ops/os-dgtl-ltd-migration`, both off `main@d35f3d4`). CommandPalette was rendered once inside HOME's page, so the shortcut and search box existed on exactly one route; the mount moved into CoreShell, which makes ⌘K work on every Core surface (verified on /companies, /opportunities, /chat, /operations/outbox). Domain work: the app answers on `dgtlmag.com` (canonical) and `dgtl.chat`, and will gain `os.dgtl.ltd` as canonical — the `dgtl.ltd` **apex stays free** for the briefed corporate placeholder, and `dgtlmag.com` is demoted rather than retired because every unsubscribe link already delivered points at it (RFC 8058 one-click, so `/api/*` keeps being **served**, not redirected — a 301 can be downgraded to GET). Audit found two things worth fixing regardless of the migration: **`PUBLIC_APP_URL` was a hardcoded literal in docker-compose**, so the VPS `.env` could not change the app's identity at all; and **25 admin form handlers built their 303 target from `PUBLIC_APP_URL`**, which — because the session cookie is host-scoped — bounced any operator working on dgtl.chat to dgtlmag.com and dropped them at the login page mid-workflow (the same bug fixed for login/logout in PR #33, which never reached these routes). Both fixed via a shared `lib/http/redirects.js`; the Stage 6.5 guard now walks the whole admin route family so a new route cannot reintroduce it. Stripe returns now prefer the request origin so funnel buyers come back to their funnel. ⚠️ Surfaced and **not** caused by this work: `dgtlmag.com` expires **2026-08-28 with auto-renew OFF** — losing it breaks one-click unsubscribe, which is a CASL/CAN-SPAM failure. Renewal is the first prerequisite in `docs/operations/os-dgtl-ltd-migration-runbook.md`. Verified: 499/499 tests, production build, helpers proven to emit relative Locations with query params intact.
- **2026-08-17 — DGTL.chat terminal mode implemented on `feat/dgtl-chat-terminal`** (on top of `eda2e76`, the unclaimed-host app-identity fix): a second surface on the same Stage 6 thread that runs the SAME registry tools as typed commands, deterministically, with no model in the loop. `useChatSession` now owns thread state once and is handed to whichever surface renders, so switching modes re-renders rather than re-fetches and a message can neither duplicate nor go missing; `ChatSurface` became presentational; `chatClient` is the single server seam and adds the AbortController, request timeout and post-cancel thread reconciliation the prior code lacked (the server turn continues under the CAS, so it is polled back to idle rather than assumed stopped). `commandGrammar` is pure — no JSX, no fetch — and splits every line into local / read / turn / unknown; only `ask` and `prepare` spend tokens and both announce first, an unrecognised line is refused for free and a second Enter opts in. New `POST /api/core/chat/commands` reaches a strict **subset** of `handleTurn`: `runReadCommand` refuses any non-`read` classification (so it can never produce an ActionProposal), `requireTool` fails closed on unknown or unadvertised ids, `validateToolArgs` makes a client-supplied `teamId` an error, team comes from the session, and every call writes the same `assistant_tool_runs` audit row a model tool call writes. Read commands got their own 60/min bucket so `home` cannot exhaust the model-turn allowance. No `dangerouslySetInnerHTML` and no typed `confirm` — confirmation stays a real button on the same `requireCoreWrite` route. Demo mode is a hard latch in the hook (every server call refuses while set). Only command history and the mode preference persist locally; never business data. **Two defects found in review and fixed in the same commit:** the terminal input was `disabled` while busy, so it fired no keydown and lost focus and the advertised ⌃C cancel silently did nothing for exactly the window in which it mattered (now `readOnly` with an explicit busy branch); and the chat composer stopped clearing after a send once the draft moved up to `ChatWorkspace` to survive a mode switch, leaving the sent text in the box and trivially double-sendable. Both are pinned by new tests. Verified from `platform/`: **530/530 tests** and `npm run build` exit 0. Draft PR only — not merged, not deployed.
