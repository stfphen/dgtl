...
/**
 * Shared lead form for the showcase template. Every submission goes through
 * the existing public lead pipeline (/api/leads); tagging rides on the fields
 * that survive sanitizePublicLeadInput (category, packageId, contactTitle,
 * notes), assembled by the caller's mapPayload(values).
 *
 ...
 */
export default function ShowcaseLeadForm({
  fields,
  submitLabel,
  successMessage,
  tenant,
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setStatus("error");
      setNote("That didn't go through. Try again, or reach us on Instagram @dmtv.");
    }
  }

  function renderField(field) {
    const { name, label, type = "text", required, placeholder, options, span } = field;
    const id = `sc-${name}-${label.replace(/\s+/g, "-").toLowerCase()}`;
