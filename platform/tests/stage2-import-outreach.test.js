...
test("engineering worker refuses an unauthorized transport",async()=>{const repository=new MemoryStage2Repository();const service=outreach(repository,{transport:{name:"production",send:async()=>({ok:true})}});await assert.rejects(service.processOutbox(),/not explicitly authorized/);});
