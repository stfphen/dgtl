/**
 * Thin authenticated client for the DGTL Worklog API (office.dgtl.at).
 *
 ...
 *   2. Worklog runs under Passenger on Hostinger shared hosting, which caps
 *      concurrent connections. Requests are therefore serialised through a
 *      queue with a minimum gap, so a bulk task import cannot stampede it.
 *   3. Credentials come from the environment and never reach a log line, an
...
class WorklogClient {
  ...
  constructor({ baseUrl, email, password } = {}) {
    this.baseUrl = (baseUrl || process.env.WORKLOG_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, '');
    this.email = email || process.env.WORKLOG_EMAIL;
    this.password = password || process.env.WORKLOG_PASSWORD;

    if (!this.email || !this.password) {
      throw new WorklogError(
        'Missing credentials. Set WORKLOG_EMAIL and WORKLOG_PASSWORD (see .env.example).',
      );
    }
  }

  /** Serialise every outbound request, spacing them by MIN_REQUEST_GAP_MS. */
  #schedule(fn) {
    ...
    this.#loginPromise = (async () => {
      ...
      if (!res.ok) {
        ...
        const hint = res.status === 401
          ? 'Check WORKLOG_EMAIL and WORKLOG_PASSWORD in .env, and that the account is active.'
          : '';
        throw new WorklogError(
  ...
  async call(method, path, body, { _retried = false } = {}) {
    ...
    // delete sessions server-side). One re-auth, then give up.
